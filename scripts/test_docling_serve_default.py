...
# ]
# ///
"""Test docling-serve with the default pipeline (no Qwen3-VL) for comparison.

Sends the same PDF as test_docling_serve_qwen.py but uses docling's stock
OCR / layout / table engines. Saves the result to scripts/test_output_default.json
so it can be diffed against the qwen run.
"""
...
def submit_task(pdf_url: str, base_url: str, client: httpx.Client) -> str:
    ...
    console.print("[bold blue]Submitting conversion task (default pipeline)...[/]")
...
def main() -> None:
    ...
    console.print(Panel(
        f"[bold]docling-serve DEFAULT pipeline test[/]\n\n"
    ...
    out_path = Path("scripts/test_output_default.json")
