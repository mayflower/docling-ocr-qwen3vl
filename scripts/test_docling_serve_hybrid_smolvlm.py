...
# ]
# ///
"""Hybrid pipeline with SmolVLM picture description (instead of Qwen3-VL).

OCR + tables / layout same as test_docling_serve_hybrid_pic.py — only the
picture description engine swapped from "qwen3vl" to the upstream SmolVLM
preset. Saves to scripts/test_output_hybrid_smolvlm.json so it can be A/B'd
against scripts/test_output_hybrid_pic.json.
"""
...
def submit_task(pdf_url: str, base_url: str, client: httpx.Client) -> str:
    ...
    console.print("[bold blue]Submitting (hybrid + SmolVLM picture description)...[/]")
...
def main() -> None:
    ...
    console.print(Panel(
        f"[bold]docling-serve HYBRID + SmolVLM picture description test[/]\n\n"
        f"OCR:     qwen3vl_ocr (Qwen3-VL)\n"
        f"Pictures: smolvlm preset (SmolVLM-256M-Instruct)\n"
    ...
    out_path = Path("scripts/test_output_hybrid_smolvlm.json")
