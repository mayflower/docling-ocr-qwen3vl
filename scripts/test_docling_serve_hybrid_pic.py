...
# ]
# ///
"""Hybrid pipeline with Qwen3-VL picture description.

OCR + picture description from Qwen3-VL, layout + tables from default docling.
Saves to scripts/test_output_hybrid_pic.json.
"""
...
def submit_task(pdf_url: str, base_url: str, client: httpx.Client) -> str:
    ...
    console.print("[bold blue]Submitting (hybrid + qwen3vl picture description)...[/]")
...
def main() -> None:
    ...
    console.print(Panel(
        f"[bold]docling-serve HYBRID + picture description test[/]\n\n"
        f"OCR:     qwen3vl_ocr (Qwen3-VL)\n"
        f"Pictures: qwen3vl (Qwen3-VL)\n"
    ...
    out_path = Path("scripts/test_output_hybrid_pic.json")
